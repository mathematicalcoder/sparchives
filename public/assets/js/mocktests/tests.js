...
function showQuestions(details, questions, hps) {
  for (let i = 0; i < questions.length; i++) {
    ...
    if (questions[i].type == "essay") {
      element.innerHTML = `<td colspan="2"><span class="mtHeading">Question ${i+1} (${questions[i].points} point/s).</span> ${questions[i].question}<br><span id="p${i+1}Status"><textarea id="p${i+1}Answer" placeholder="Enter answer"></textarea></span></td>`
    } else {
      element.innerHTML = `<td width="200px" id="p${i+1}Status"><input type="${questions[i].type}" id="p${i+1}Answer" placeholder="Enter answer"></td> <td><span class="mtHeading">Question ${i+1} (${questions[i].points} point/s).</span> ${questions[i].question}</td>`
    }
  ...
  <ol>
    ...
    <li>Answer all questions without any notes, resources, calculators, etc. (Calculators and Jamovi are allowed in Statistics mock tests unless otherwise stated.)</li>
